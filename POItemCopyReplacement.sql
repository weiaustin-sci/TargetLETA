...
-- Description:
--		Purchase Item Copy For Replacement
-- =============================================
If Object_Id ( 'dbo.POItemCopyReplacement', 'P' ) Is Not Null
    Drop Procedure dbo.POItemCopyReplacement;
Go

Create Procedure [dbo].[POItemCopyReplacement]
	(
	  @CopyType			Numeric(1,0)	--複製類型: 1. Copy to Seq1; 2. Copy to Seq2
	 ,@ToPoID			VarChar(13)		--採購母單單號
	 ,@FromPoID			VarChar(13)		--採購母單單號
	 ,@FromSeq1			VarChar(3)		--採購大項
	 ,@FromSeq2			VarChar(2)		--採購小項
	 ,@FinalSeq1		VarChar(3)		--
	 ,@CopySuppID		VarChar(6)		--新Supplier
	 ,@CopySCIRefNo		VarChar(30)	=''	--新SCIRefNo
	 ,@UserID			VarChar(10) = ''
...
Begin
	...
	Begin
		Create Table #tmpSeq
			(  Sel Bit, Seq1 VarChar(3), Seq2 VarChar(2), SciRefNo VarChar(30), SuppID VarChar(6)
			 , Primary Key (Seq1, Seq2)
			);
		
		Insert Into #tmpSeq
			(Sel, Seq1, Seq2, SciRefNo, SuppID)
			Select 1 as Sel, PO_Supp_Detail.Seq1, PO_Supp_Detail.Seq2, PO_Supp_Detail.SciRefNo, PO_Supp.SuppID
			  From dbo.PO_Supp_Detail
			  Left Join dbo.PO_Supp
				On     PO_Supp.ID = PO_Supp_Detail.ID
				   And PO_Supp.Seq1 = PO_Supp_Detail.Seq1
			 Where PO_Supp_Detail.ID = @FromPoID
			   And PO_Supp_Detail.Seq1 = @FromSeq1
			   And (   (@CopyType = 1)
					Or (@CopyType = 2 And PO_Supp_Detail.Seq2 = @FromSeq2)
				   )
			 Order by PO_Supp_Detail.ID, PO_Supp_Detail.Seq1, PO_Supp_Detail.Seq2;
	End;
	----------------------------------------------------------------------
	...
	Declare @tmpPo_Supp Table
		(  RowID BigInt Identity(1,1) Not Null, Seq1 VarChar(3), SuppID VarChar(6), ShipTermID VarChar(5), PayTermAPID VarChar(5)
		 , Remark NVarChar(Max), [Description] NVarChar(Max), CompanyID Numeric(2,0)
		);
	...
	Declare @tmpPo_Supp_Detail Table
		(  RowID BigInt Identity(1,1) Not Null, Seq1 VarChar(3), Seq2 VarChar(2), RefNo VarChar(36), SCIRefNo VarChar(30)
		 ...
		 , Remark NVarChar(Max), Special NVarChar(120), Width Numeric(5,2), ColorDetail NVarchar(200)
		 , SystemETD DATE
		 , Spec NVarChar(Max), FactoryID VarChar(10), Keyword_Original VarChar(Max)
		);
	Declare @tmpPo_Supp_DetailRowID Int;	--Row ID
	Declare @tmpPo_Supp_DetailRowCount Int;	--總資料筆數

	Declare @tmpPo_Supp_Detail_OrderList Table
		(RowID BigInt Identity(1,1) Not Null, Seq1 VarChar(3), Seq2 VarChar(2), OrderID VarChar(13));
	...
	Insert Into @tmpPo_Supp
		(Seq1, SuppID, ShipTermID, PayTermAPID, Remark, [Description], CompanyID)
		Select PO_Supp.Seq1, PO_Supp.SuppID, PO_Supp.ShipTermID, PO_Supp.PayTermAPID
			 , PO_Supp.Remark, PO_Supp.Description, PO_Supp.CompanyID
		  From dbo.PO_Supp
		 Where PO_Supp.ID = @FromPoID
		   And PO_Supp.Seq1 = @FromSeq1
		   And Exists (Select * From #tmpSeq Where #tmpSeq.Sel = 1 And #tmpSeq.Seq1 = PO_Supp.Seq1)
		 Order by Seq1;

	Insert Into @tmpPo_Supp_Detail
		(  Seq1, Seq2, RefNo, SCIRefNo, FabricType, Price, UsedQty, Qty, POUnit, FinalETD, EstETA, ShipModeID
		 , ColorID, SuppColor, Remark, Special, Width, ColorDetail
		 , Spec, FactoryID, Keyword_Original
		)
		Select PO_Supp_Detail.Seq1, PO_Supp_Detail.Seq2, PO_Supp_Detail.RefNo, iif(@CopySCIRefNo='', PO_Supp_Detail.SCIRefNo,@CopySCIRefNo), PO_Supp_Detail.FabricType
			 , PO_Supp_Detail.Price, PO_Supp_Detail.UsedQty, PO_Supp_Detail.Qty, PO_Supp_Detail.POUnit
			 , PO_Supp_Detail.FinalETD, PO_Supp_Detail.EstETA, PO_Supp_Detail.ShipModeID
			 , po3Spec.Color, PO_Supp_Detail.SuppColor
			 , PO_Supp_Detail.Remark, PO_Supp_Detail.Special, PO_Supp_Detail.Width, PO_Supp_Detail.ColorDetail
			 , PO_Supp_Detail.Spec, PO_Supp_Detail.FactoryID, PO_Supp_Detail.Keyword_Original
		  From dbo.PO_Supp_Detail
		  outer apply dbo.GetPo3Spec(PO_Supp_Detail.ID, PO_Supp_Detail.Seq1, PO_Supp_Detail.Seq2) po3Spec
		 Where PO_Supp_Detail.ID = @FromPoID
		   And PO_Supp_Detail.Seq1 = @FromSeq1
		   And Exists (Select * From #tmpSeq Where #tmpSeq.Sel = 1 And #tmpSeq.Seq1 = PO_Supp_Detail.SEQ1 And #tmpSeq.Seq2 = PO_Supp_Detail.SEQ2)
		   And (   (@CopyType = 1)
				Or (@CopyType = 2 And PO_Supp_Detail.Seq2 = @FromSeq2)
			   )
		 Order by Seq1, Seq2;
	...
	Insert Into @tmpPo_Supp_Detail_Spec
		...
		Where po4.ID = @FromPoID
			And po4.Seq1 = @FromSeq1
			And Exists (Select * From #tmpSeq Where #tmpSeq.Sel = 1 And #tmpSeq.Seq1 = po4.Seq1 And #tmpSeq.Seq2 = po4.Seq2)
			And (   (@CopyType = 1)
				Or (@CopyType = 2 And po4.Seq2 = @FromSeq2)
			   )
		 Order by Seq1, Seq2;
	...
	Insert Into @tmpPo_Supp_Detail_OrderList
		(Seq1, Seq2, OrderID)
		Select PO_Supp_Detail_OrderList.Seq1, PO_Supp_Detail_OrderList.Seq2, PO_Supp_Detail_OrderList.OrderID
		  From dbo.PO_Supp_Detail_OrderList
		 Inner Join #tmpSeq
			On	   #tmpSeq.Sel = 1
			   And #tmpSeq.Seq1 = PO_Supp_Detail_OrderList.Seq1
			   And #tmpSeq.Seq2 = PO_Supp_Detail_OrderList.Seq2
		 Where PO_Supp_Detail_OrderList.ID = @FromPoID
		   And PO_Supp_Detail_OrderList.Seq1 = @FromSeq1
		   And (   (@CopyType = 1)
				Or (@CopyType = 2 And PO_Supp_Detail_OrderList.Seq2 = @FromSeq2)
			   )
		 Order by Seq1, Seq2;
	-------------------------------------------------
	...
	Declare @numSeq2_New Numeric(3,0);
	...
	Begin
		...
		--------------------------------------
		SET @Seq1_New = @FinalSeq1;
		--------------------------------------
		--若新Supplier為空白則，則保留原Supplier
		Set @SuppID_New = @SuppID;
		If IsNull(@CopySuppID, '') != ''
		Begin
			Set @SuppID_New = @CopySuppID;
		End;

		Select top 1 @FabricType = FabricType From @tmpPo_Supp_Detail Where Seq1 = @Seq1
		...
		If @SuppLockDate Is Not Null
		Begin
			Set @ErrorMessage += 'Seq1:' + @Seq1 + '-' + @SuppID_New + '- [Supplier Code] Lock!' + Char(13);
		End;
		--------------------------------------
		--若非COPY為新廠代 則保留原來Remark
		If IsNull(@CopySuppID, '') != ''
		Begin
		...
		If IsNull(@PayTermAPID, '') = ''
		Begin
			Set @ErrorMessage += 'Seq1:' + @Seq1 + '-' + @SuppID_New + '- [Payment Term] cannot empty !!' + Char(13);
		End;
		If IsNull(@ShipTermID, '') = ''
		Begin
			Set @ErrorMessage += 'Seq1:' + @Seq1 + '-' + @SuppID_New + '- [Shipping Term] cannot empty !!' + Char(13);
		End;
		If IsNull(@CompanyID, 0) = 0
		Begin
			Set @ErrorMessage += 'Seq1:' + @Seq1 + '-' + @SuppID_New + '- [Company] cannot empty !!' + Char(13);
		End;

		Set @numSeq2_New = 0;
		Select @numSeq2_New = isnull(Max(Seq2),0) From Po_Supp_Detail Where ID = @ToPoID and Seq1 = @FinalSeq1
		...
		Begin
			...
			If @@RowCount = 0
			Begin
				Set @ErrorMessage += 'Seq1:' + @Seq1 + ' Seq2:' + @Seq2 + ' SCI Ref#:' + @SCIRefNo + '- Fabric not found!!' + Char(13);
				delete @tmpPo_Supp_Detail where RowID = @tmpPo_Supp_DetailRowID;
				Set @tmpPo_Supp_DetailRowID += 1;
				continue;
			End;
			...
			Begin
				Set @ErrorMessage += 'Seq1:' + @Seq1 + ' Seq2:' + @Seq2 + ' SCI Ref#:' + @SCIRefNo_New + '- Fabric Lock or Junk,Please Check!!' + Char(13);
				delete @tmpPo_Supp_Detail where RowID = @tmpPo_Supp_DetailRowID;
				Set @tmpPo_Supp_DetailRowID += 1;
				continue;
			End;
			...
			set @SystemETD = DATEADD(DAY, dbo.GetMtlLT(@SCIRefNo_New, @Category, @SuppID_New, @StyleID, @ToBrandID, @SeasonID, @FtyCountry, @OrderTypeID, @ColorID), Getdate());
	...
	Begin Try
		...
		Insert Into dbo.PO_Supp
			...
			Select @ToPoID, Seq1, SuppID, ShipTermID, PayTermAPID, Remark, [Description], CompanyID
				 , @UserID, @AddDate
			  From @tmpPo_Supp where not EXISTS(SELECT * FROM PO_Supp WHERE PO_Supp.ID = @ToPoID and PO_Supp.SEQ1 = [@tmpPo_Supp].Seq1);
		
		Insert Into dbo.PO_Supp_Detail
			(  ID, Seq1, Seq2, RefNo, SCIRefNo, FabricType, Price, UsedQty, Qty, POUnit, ShipModeID
			 , SuppColor, Remark, Special, Width, ColorDetail
			 , SystemETD, FinalETD
			 , Spec, FactoryID, AddName, AddDate, Keyword_Original
			)
			Select @ToPoID, Seq1, Seq2, RefNo, SCIRefNo, FabricType, Price, UsedQty, Qty, POUnit, ShipModeID
				 , SuppColor, Remark, Special, Width, ColorDetail
				 , SystemETD, SystemETD
				 , Spec, FactoryID, @UserID, @AddDate, Keyword_Original
			  From @tmpPo_Supp_Detail where not EXISTS(SELECT * FROM PO_Supp_Detail WHERE PO_Supp_Detail.ID = @ToPoID and PO_Supp_Detail.SEQ1 = [@tmpPo_Supp_Detail].Seq1 and PO_Supp_Detail.SEQ2 = [@tmpPo_Supp_Detail].Seq2);

		Insert Into dbo.PO_Supp_Detail_OrderList
			(ID, Seq1, Seq2, OrderID, AddName, AddDate)
			Select @ToPoID, Seq1, Seq2, OrderID
				 , @UserID, @AddDate
