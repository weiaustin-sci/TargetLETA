...
--		Transfer To PO
-- =============================================
If Object_Id ( 'dbo.TransferToPO_3_InsertForThread', 'P' ) Is Not Null
    Drop Procedure dbo.TransferToPO_3_InsertForThread;
Go

Create Procedure [dbo].[TransferToPO_3_InsertForThread]
	(
...
Begin
	...
	Declare @OldVersion varchar(5);
	Declare @NewVersion varchar(5);
	...
	Begin Try
		...
		Select @OldVersion = ThreadVersion From PO Where ID = @PoID

		--更新ThreadVersion
		Update dbo.PO 
		Set ThreadVersion = s.ThreadVersion
		From (
			select top 1 ThreadVersion = Version
		...
		Select @NewVersion = ThreadVersion From PO Where ID = @PoID

		--紀錄ThreadVersion的異動
		Insert Into dbo.TradeHIS_Purchase (TableName, HisType, SourceID, OldValue, NewValue, AddName, AddDate)
		Values ('PO', 'ThreadVersion', @PoID, @OldVersion, @NewVersion, @UserID, GETDATE())

		Delete From dbo.PO_Supp Where ID = @PoID And Seq1 = 'A3';
		Delete From dbo.PO_Supp_Detail Where ID = @PoID And Seq1 = 'A3';
		...
		Begin
			...
			insert into TradeHIS_Purchase (TableName, SourceID, NewValue, OldValue, AddName, AddDate)
					VALUES ('tmpPO_Supp_Detail', @PoID, @tmpPo_Supp_DetailRowCount, @tmpPo_Supp_DetailRowID, @UserID, @NowDateTime)
			...
			Begin
				Select @Seq2 = #tmpPO_Supp_Detail.Seq2
					 ...
					 , @IsFOC = Isnull(f.IsFOC, 0)
					 , @IsForOtherBrand = isnull(#tmpPO_Supp_Detail.IsForOtherBrand, 0)
				...
				Begin
					Insert Into dbo.PO_Supp_Detail
						(  ID, Seq1, Seq2, RefNo, SciRefNo, FabricType, Price, UsedQty, Qty, POUnit, Complete
						 ...
						 , AddName, AddDate, Spec, Remark_Shell, IsForOtherBrand
						)
					Values
						(  @PoID, @Seq1, @Seq2, @RefNo, @SciRefNo, @FabricType, @Price, @UsedQty, @Qty, @POUnit, @Complete
						 ...
						 , @UserID, @NowDateTime, @Spec, @Remark_Shell, @IsForOtherBrand
						);
				...
				Begin
					...
					Update po3
						Set Qty = IIF(@Sel = 1, @Qty, Qty)
							...
							, EditName = @UserID
							, EditDate = @NowDateTime
							, Junk = IIF(@Sel = 1, IIF((@Qty = 0 And @IsFOC = 0) Or (@FOC = 0 And @IsFOC = 1), 1, 0), Junk)
...
Go
Exec Trade.dbo.CopyProcedureToDataBase 'MemDB', 'TransferToPO_3_InsertForThread';
Go